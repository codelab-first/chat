...
/* ✅ 로그인/회원가입 전용 버튼 */
const RoundedButton = styled.button<{ bgcolor?: string, fullWidth?: boolean }>`
  display: block;
...
const AuthForm: React.FC<Props> = ({ form = "login" }) => {
  ...
  return (
    <>
      ...
      {form === "login" ? (
        <RoundedButton onClick={login} fullWidth={true} >Login</RoundedButton>
      ) : (
        <RoundedButton onClick={join} fullWidth={true}>Join</RoundedButton>
      )}
